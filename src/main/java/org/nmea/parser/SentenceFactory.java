...
public final class SentenceFactory {
   ...
   * type. The returned instance must be cast in to correct sentence interface,
   * for which the type should first be checked by using the
   ...
   * mainly intended to be used when custom parsers have been registered in the
   * factory. The returned instance needs to be cast to corresponding sentence
   * interface.
   *
   ...
   * chosen sentence type. That is, each sentence type can have only one parser
  ...
  /**
   * Unregisters a parser class, regardless of sentence type(s) it is registered
   * for.
   *
