 ...
 * @see ch.keybridge.lib.nmea.util.Date
 */
public final class Time {
  ...
  /**
   * Creates a new instance of <code>Time</code> using the current system time.
   */
  public Time() {
  ...
  /**
   * Creates a new instance of <code>Time</code> based on given String. Assumes
   * the <code>hhmmss.sss</code> formatting used in NMEA sentences.
   *
  ...
  /**
   * Set the time by {@link java.util.Date}. The date information of is ignored,
   * only hours, minutes and seconds are relevant. Notice also that time zone
   * offset is not affected by this method because {@link java.util.Date} does
   * not contain zone offset.
   *
   * @param d Date
   */
  public void setTime(Date d) {
    ...
    double secondsTemp = cal.get(Calendar.SECOND) + (cal.get(Calendar.MILLISECOND) / 1000.0);
  ...
  public Date toDate(Date d) {
    ...
    double secondsTemp = getSeconds();
    int fullSeconds = (int) Math.floor(secondsTemp);
    int milliseconds = (int) Math.round((secondsTemp - fullSeconds) * 1000);
  ...
  /**
   * Returns the String representation of <code>Time</code>. Formats the time in
   * <code>hhmmss.sss</code> format used in NMEA 0183 sentences. Seconds are
   * presented with three decimals regardless of precision returned by
   * {@link #getSeconds()}.
   */
  @Override
