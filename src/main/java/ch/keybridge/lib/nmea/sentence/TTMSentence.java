...
import ch.keybridge.lib.nmea.util.AcquisitionType;
import ch.keybridge.lib.nmea.util.TargetStatus;
...
public interface TTMSentence extends TimeSentence {
   ...
   * radar works out firm data of the target. At first the calculated course and
   * speed are rough and varies a lot; with time they settle at which point the
   * target becomes TRACKING. A target no longer detected becomes LOST before
   * TTM sentences will not be sent at all for the target.
   *
   ...
   *
   * @param number The number in the range 0 to 99 inclusive.
   */
   ...
   *
   * @param bearing True bearing in degrees.
   */
   ...
   *
   * @param speed Speed in kts (nautical miles per hour).
   */
   ...
   * Set the time to CPA.
   *
   * @param minutes Time to CPA in minutes.
   */
   ...
   *
   * @param name The name as a string, probably not too long...
   */
   ...
   * A target may be used to calculate own ship position.
   *
   * @param isReference True if this target is used to calculate own ship
   *                    position.
   */
