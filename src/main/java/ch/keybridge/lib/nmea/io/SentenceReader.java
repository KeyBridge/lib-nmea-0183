...
/**
 * Sentence reader detects supported NMEA 0183 sentences from the specified data
 * source and dispatches them to registered listeners as sentence events. Each
 * event contains a parser for the read sentence.
 * <p>
 * Parsers dispatched by reader are created using
 * {@link net.sf.marineapi.nmea.parser.SentenceFactory} class, where you can
 * also register your own custom parsers.
 *
 * @author Kimmo Tuukkanen
 * @see ch.keybridge.lib.nmea.event.AbstractSentenceListener
 * @see ch.keybridge.lib.nmea.event.SentenceListener
 * @see ch.keybridge.lib.nmea.event.SentenceEvent
...
public class SentenceReader {
  ...
  /**
   * Adds a {@link ch.keybridge.lib.nmea.event.SentenceListener} that wants to
   * receive all sentences read by the reader.
   *
   * @param listener {@link ch.keybridge.lib.nmea.event.SentenceListener} to be
   *                 registered.
   * @see ch.keybridge.lib.nmea.event.SentenceListener
   */
  ...
  /**
   * Adds a {@link ch.keybridge.lib.nmea.event.SentenceListener} that is
  ...
  /**
   * Handles an exception by passing it to ExceptionHandler. If no handler is
   * present, logs the error at level WARNING.
   *
  ...
  /**
   * Sets the DatagramSocket to be used as data source. If reader is running, it
  ...
  /**
   * Set listener for any data that is not recognized as NMEA 0183. devices and
   * environments that produce mixed content with both NMEA and non-NMEA data.
   *
  ...
  /**
   * Sets the InputStream to be used as data source. If reader is running, it is
   * first stopped and you must call {@link #start()} to resume reading.
   *
